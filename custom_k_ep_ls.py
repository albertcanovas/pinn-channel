...
"""
    Equations to model fluid flow using a k-epsilon RANS model
"""
...
class kEpsilonInit(PDE):
    """
        PDE to add a loss to work as an initialization values. 
        The initial values for u,v and p are hard coded in the function.
    """
    def __init__(self, nu=1, rho=1):
        """
            Defines the PDE equations for the 5 variables (u, v, p, k and epsilon) to be used in the training process for a 
            :param nu: kinematic viscoisty
            :param rho: density
        """
        # set params
        nu = Number(nu)
        ...
        # flow initialization
        C_mu = 0.09
        u_avg = 1.41  # Approx average velocity
        Re_d = (
            u_avg * rho / nu
...
class kEpsilon(PDE):
    """
        PDE to define a 2D implementation of the standard k-epsilon RANS model.
        It derives from the base modulus PDE class
    """
    def __init__(self, nu=1, rho=1):
        """
            Defines the PDE equations of a standard k-epsilon model for continuity, momentum in x and y and transport equations for k and epsilon
            :param nu: kinematic viscoisty
            :param rho: density
        """
        # set params
...
class kEpsilonLSWF(PDE):
    """
        PDE to define a 2D implementation of the Launder-Spalding Wall Function
        It derives from the base modulus PDE class
    """
    def __init__(self, nu=1, rho=1):
        """
            Defines the PDE equations of the Launder-Spalding Wall Function (LSWF)
            :param nu: kinematic viscoisty
            :param rho: density
        """
        # set params
