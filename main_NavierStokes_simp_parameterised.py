...
""" 
 * File:   main_NavierStokes_simp_parametrized.py
...
from auxiliaryFunctions import LoadCsvData, AddDataConstraints, NoSlipWall2D
...
def run(cfg: ModulusConfig) -> None:
    ...
    # Defining the range of inlet velocities 
    u_inlet1_vec = [1.0,2.0]
    u_inlet2_vec = [0.1,0.2]
    ...
    lambda_wall = 1
    wf_pt_bc = NoSlipWall2D(nodes,wallbc,lambda_wall,cfg.batch_size.short,pr)
    domain.add_constraint(wf_pt_bc, "WFbc")

    wf_pt_de = NoSlipWall2D(nodes,wallde,lambda_wall,cfg.batch_size.short,pr)
    domain.add_constraint(wf_pt_de, "WFde")

    wf_pt_fg = NoSlipWall2D(nodes,wallfg,lambda_wall,cfg.batch_size.short,pr)
    domain.add_constraint(wf_pt_fg, "WFfg")

    wf_pt_hi = NoSlipWall2D(nodes,wallhi,lambda_wall,cfg.batch_size.short,pr)
    domain.add_constraint(wf_pt_hi, "WFhi")

    wf_pt_jk = NoSlipWall2D(nodes,walljk,lambda_wall,cfg.batch_size.short,pr)
    domain.add_constraint(wf_pt_jk, "WFjk")

    wf_pt_kl = NoSlipWall2D(nodes,wallkl,lambda_wall,cfg.batch_size.short,pr)
    domain.add_constraint(wf_pt_kl, "WFkl")

    wf_pt_lm = NoSlipWall2D(nodes,walllm,lambda_wall,cfg.batch_size.wf_pt,pr)
    domain.add_constraint(wf_pt_lm, "WFlm")

    wf_pt_no = NoSlipWall2D(nodes,wallno,lambda_wall,cfg.batch_size.wf_pt,pr)
    domain.add_constraint(wf_pt_no, "WFno")

    wf_pt_op = NoSlipWall2D(nodes,wallop,lambda_wall,cfg.batch_size.tiny,pr)
    domain.add_constraint(wf_pt_op, "WFop")

    wf_pt_pq = NoSlipWall2D(nodes,wallpq,lambda_wall,cfg.batch_size.short,pr)
    domain.add_constraint(wf_pt_pq, "WFpq")

    wf_pt_qa = NoSlipWall2D(nodes,wallqa,lambda_wall,cfg.batch_size.short,pr)
    ...
    data_path = "Data/Valdata2D_re150-2,00_0,155_1,44.csv"

    invar_test = LoadCsvData(data_path,invar)
    outvar_test = LoadCsvData(data_path,outvar)

    invar_test.update({"u_inlet1": np.full_like(invar_test["x"], 2.2)})
    ...
    data_path = "Data/Valdata2D_re150-1,50_0,155_1,44.csv"

    invar_test = LoadCsvData(data_path,invar)
    outvar_test = LoadCsvData(data_path,outvar)

    invar_test.update({"u_inlet1": np.full_like(invar_test["x"], 1.50)})
    invar_test.update({"u_inlet2": np.full_like(invar_test["x"], 0.155)})
    invar_test.update({"u_inlet3": np.full_like(invar_test["x"], 1.44)})
    
    validator2 = PointwiseValidator(
            nodes=nodes,
            invar=invar_test,
            true_outvar=outvar_test,
            batch_size=1024,
    ...
    domain.add_validator(validator2)

    #---- Inference more variables of interest
    inference = PointwiseInferencer(
        nodes=nodes,
        invar=invar_test,
        output_names=["u", "v", "p","continuity","momentum_x","momentum_y"],
